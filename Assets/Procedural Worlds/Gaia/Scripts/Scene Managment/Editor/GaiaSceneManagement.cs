...
namespace Gaia
{
    ...
    public static class GaiaSceneManagement
    {
        ...
        private static GameObject firstPersonController;
        private static GameObject thirdPersonController;
        private static GameObject flyCamController;
        private static GameObject flyCamControllerUI;
        private static GameObject carController;
                            ...
                            if (settings != null)
                            {
                                    ...
                                    GaiaConstants.GlobalSystemMode.ThirdParty)
                                {
                                    GaiaWater.GetProfile(
                                        GaiaGlobal.Instance.SceneProfile.m_selectedWaterProfileValuesIndex,
                    ...
                    if (GaiaGlobal.Instance != null)
                    {
                            ...
                            "Yes", "No"))
                        {
                            ...
                            if (settings != null)
                            {
                                ...
                                GaiaWater.GetProfile(GaiaGlobal.Instance.SceneProfile.m_selectedWaterProfileValuesIndex, GaiaGlobal.Instance.SceneProfile, true, false);
                                GaiaGlobal.Instance.SceneProfile.DefaultWaterSet = true;
            ...
            if (profile != null)
            {
                if (GaiaGlobal.Instance != null)
                {
                    if (!GaiaGlobal.Instance.SceneProfile.DefaultLightingSet)
                    {
                        ...
                        sceneProfile.m_lightSystemMode = GaiaGlobal.Instance.SceneProfile.m_lightSystemMode;
                        sceneProfile.m_waterSystemMode = GaiaGlobal.Instance.SceneProfile.m_waterSystemMode;
        ...
        /// Create a player
        /// </summary>
        public static GameObject CreatePlayer(GaiaSettings gaiaSettings, bool instantiateCustomSetup, string overrideName = "", bool spawnAtLocation = false, GameObject customPlayerObject = null, Camera customPlayerCamera = null, bool updateAutoDOF = false, bool useGaiaUIAndPhotoMode = true)
        {
            ...
            Vector3 location = GetLocation(playerPrefabName, spawnAtLocation, sceneinfo, null, true);
            ...
            if (playerPrefabName == "Custom")
            {
                if (instantiateCustomSetup)
                {
                    ...
                    {
                        GameObject customCameraObject = GameObject.Instantiate(customPlayerCamera.gameObject);
                        Camera cameraComponent = customCameraObject.GetComponent<Camera>();
            ...
            else
            {
                ...
                {
                    if (GaiaScenePlayer.Instance  == null)
                    {
                        //Create an instance of the Gaia scene player if it does not exist yet
                        GaiaUtils.GetPlayerObject();
                    }
                    ...
                    if (scenePlayer != null)
                    {
                        if (scenePlayer.m_gaiaUI == null && useGaiaUIAndPhotoMode && gaiaSettings.m_currentController != GaiaConstants.EnvironmentControllerType.XRController && gaiaSettings.m_currentController != GaiaConstants.EnvironmentControllerType.None && gaiaSettings.m_currentController != GaiaConstants.EnvironmentControllerType.Custom)
                        {
                            ...
                            {
                                GaiaGlobal.Instance.m_globalCanvas = flyCamControllerUI.GetComponent<Canvas>();
                            }
                ...
                if (dynamicLoadedTerrains && playerObj !=null)
                {
                    ...
                    if (GaiaUtils.CheckIfSceneProfileExists())
                    {
                        ...
                        sceneProfile.SaveTerrainLoaderToProfile(loader);
                    }
#endif
                }
            }
        ...
        /// <summary>
        /// Removes All Gaia player controllers
        /// </summary>
        /// <param name="playerPrefabName"></param>
        /// <param name="spawnAtLocation"></param>
        /// <param name="sceneinfo"></param>
        /// <param name="removeCamera"></param>
        /// <returns></returns>
        private static Vector3 RemoveGaiaControllers(string playerPrefabName, bool spawnAtLocation, GaiaSceneInfo sceneinfo, Camera customPlayerCamera, bool removeCamera = true)
        {
            ...
            if (removeCamera)
            {
                ...
                foreach (Camera camera in cameras)
                {
                    if (customPlayerCamera != null)
                    {
                        if (camera.gameObject != customPlayerCamera.gameObject)
                        {
                            if (spawnAtLocation)
                            {
                        ...
                        {
                            ProcessCameraComponents(camera);
                        }
                    }
                    else
                    {
        ...
        private static void ProcessCameraComponents(Camera camera)
        {
            if (camera != null)
            {
                Gaia.FreeCamera freeCamera = camera.GetComponent<Gaia.FreeCamera>();
                if (freeCamera != null)
                {
                    GameObject.DestroyImmediate(freeCamera);
                }

                Gaia.CameraController cameraController = camera.GetComponent<Gaia.CameraController>();
                if (cameraController != null)
                {
                    GameObject.DestroyImmediate(cameraController);
                }
            }
        }
        /// <summary>
...
#endif
            GaiaGlobal.FinalizeCameraObjectRuntime(cameraObject, true);
        }
        ...
        public static void CopySettingsTo(GaiaLightingProfile sourceProfile, SceneProfile newProfile)
        {
            ...
            SceneProfile currentSceneProfile = GaiaGlobal.Instance.SceneProfile;
            //Player and Controller
            newProfile.m_controllerType = currentSceneProfile.m_controllerType;
            newProfile.m_setupPostFX = currentSceneProfile.m_setupPostFX;
            newProfile.m_spawnPlayerAtCurrentLocation = currentSceneProfile.m_spawnPlayerAtCurrentLocation;
            newProfile.m_customPlayer = currentSceneProfile.m_customPlayer;
            newProfile.m_customCamera = currentSceneProfile.m_customCamera;

            //Camera Culling
            newProfile.m_enableLayerCulling = currentSceneProfile.m_enableLayerCulling;
            newProfile.CullingProfile = currentSceneProfile.CullingProfile;

            //Terrain Culling
            newProfile.m_terrainCullingEnabled = currentSceneProfile.m_terrainCullingEnabled;

            //Terrain Loading
#if GAIA_PRO_PRESENT
            newProfile.m_terrainLoaderLoadMode = currentSceneProfile.m_terrainLoaderLoadMode;
            newProfile.m_terrainLoaderMinRefreshDistance = currentSceneProfile.m_terrainLoaderMinRefreshDistance;
            newProfile.m_terrainLoaderMaxRefreshDistance = currentSceneProfile.m_terrainLoaderMaxRefreshDistance;
            newProfile.m_terrainLoaderMinRefreshMS = currentSceneProfile.m_terrainLoaderMinRefreshMS;
            newProfile.m_terrainLoaderMaxRefreshMS = currentSceneProfile.m_terrainLoaderMaxRefreshMS;
            newProfile.m_terrainLoaderFollowTransform = currentSceneProfile.m_terrainLoaderFollowTransform;
            newProfile.m_terrainLoaderLoadingBoundsRegular = currentSceneProfile.m_terrainLoaderLoadingBoundsRegular;
            newProfile.m_terrainLoaderLoadingBoundsImpostor = currentSceneProfile.m_terrainLoaderLoadingBoundsImpostor;
            newProfile.m_terrainLoaderLoadingBoundsCollider = currentSceneProfile.m_terrainLoaderLoadingBoundsCollider;
            newProfile.m_terrainLoaderDataInitialized = currentSceneProfile.m_terrainLoaderDataInitialized;
#endif
        ...
        private static void CopyFromProfileToNewProfile(SceneProfile sourceProfile, SceneProfile newProfile)
        {
            ...
            newProfile.m_lightSystemMode = sourceProfile.m_lightSystemMode;
            ...
            newProfile.m_waterSystemMode = sourceProfile.m_waterSystemMode;
            ...
            //Player and Controller
            newProfile.m_controllerType = sourceProfile.m_controllerType;
            newProfile.m_setupPostFX = sourceProfile.m_setupPostFX;
            newProfile.m_spawnPlayerAtCurrentLocation = sourceProfile.m_spawnPlayerAtCurrentLocation;
            newProfile.m_customPlayer = sourceProfile.m_customPlayer;
            newProfile.m_customCamera = sourceProfile.m_customCamera;

            //Camera Culling
            newProfile.m_enableLayerCulling = sourceProfile.m_enableLayerCulling;
            newProfile.CullingProfile = sourceProfile.CullingProfile;

            //Terrain Culling
            newProfile.m_terrainCullingEnabled = sourceProfile.m_terrainCullingEnabled;

            //Terrain Loading
#if GAIA_PRO_PRESENT
            newProfile.m_terrainLoaderLoadMode = sourceProfile.m_terrainLoaderLoadMode;
            newProfile.m_terrainLoaderMinRefreshDistance = sourceProfile.m_terrainLoaderMinRefreshDistance;
            newProfile.m_terrainLoaderMaxRefreshDistance = sourceProfile.m_terrainLoaderMaxRefreshDistance;
            newProfile.m_terrainLoaderMinRefreshMS = sourceProfile.m_terrainLoaderMinRefreshMS;
            newProfile.m_terrainLoaderMaxRefreshMS = sourceProfile.m_terrainLoaderMaxRefreshMS;
            newProfile.m_terrainLoaderFollowTransform = sourceProfile.m_terrainLoaderFollowTransform;
            newProfile.m_terrainLoaderLoadingBoundsRegular = sourceProfile.m_terrainLoaderLoadingBoundsRegular;
            newProfile.m_terrainLoaderLoadingBoundsImpostor = sourceProfile.m_terrainLoaderLoadingBoundsImpostor;
            newProfile.m_terrainLoaderLoadingBoundsCollider = sourceProfile.m_terrainLoaderLoadingBoundsCollider;
            newProfile.m_terrainLoaderDataInitialized = sourceProfile.m_terrainLoaderDataInitialized;
#endif
        ...
        private static void GetSunSettings(GaiaLightingProfileValues profile, GaiaConstants.EnvironmentRenderer renderPipeline)
        {
            try
            {
                ...
                Light light = GaiaUtils.GetMainDirectionalLight(false);
                if (light != null)
                {
