...
namespace Gaia
{
    ...
    public class GaiaPlayerEditor : PWEditor
    {
        ...
        private void GlobalSettings(bool helpEnabled)
        {
            ...
            {
                bool process = true;
                if (m_profile.m_controllerType == GaiaConstants.EnvironmentControllerType.Custom)
                {
                    if (EditorUtility.DisplayDialog(m_editorUtils.GetTextValue("CustomControllerPopupTitle"), m_editorUtils.GetTextValue("CustomControllerPopupText"), "Yes", "No"))
                    {
                        process = true;
                    }
                    else
                ...
                if (process)
                {
                    ...
                    {
                        Debug.LogWarning("You can not change player controller while you're in photo mode.");
                        return;
                    }
                    ...
                    switch (controller)
                    {
                        case GaiaConstants.EnvironmentControllerType.FirstPerson:
                            GaiaSceneManagement.CreatePlayer(m_gaiaSettings, m_profile.m_instantiateCustomControllers, GaiaConstants.playerFirstPersonName, m_profile.m_spawnPlayerAtCurrentLocation, null, null, true);
                            break;
                        case GaiaConstants.EnvironmentControllerType.FlyingCamera:
                            GaiaSceneManagement.CreatePlayer(m_gaiaSettings,m_profile.m_instantiateCustomControllers, GaiaConstants.playerFlyCamName, m_profile.m_spawnPlayerAtCurrentLocation, null, null, true);
                            break;
                        case GaiaConstants.EnvironmentControllerType.ThirdPerson:
                            GaiaSceneManagement.CreatePlayer(m_gaiaSettings,m_profile.m_instantiateCustomControllers, GaiaConstants.playerThirdPersonName, m_profile.m_spawnPlayerAtCurrentLocation, null, null, true);
                            break;
                        ...
                        {
#if GAIA_XR
                            GaiaSceneManagement.CreatePlayer(m_gaiaSettings, m_profile.m_instantiateCustomControllers, "XRController", m_profile.m_spawnPlayerAtCurrentLocation,null, null, false, false);
#else
                            EditorUtility.DisplayDialog("XR Support not enabled",
                                "The XR Controller is a default player for Virtual / Augmented Reality projects. Please open the Setup Panel in the Gaia Manager Standard Tab to enable XR Support in order to use the XR Player Controller. Please also make sure you have the Unity XR Interaction Toolkit package installed before doing so.",
                                "OK");
            ...
            if (EditorGUI.EndChangeCheck())
            {
                UIConfiguration ui = FindObjectOfType<UIConfiguration>();
                if (ui != null)
                {
                    ui.ExecuteUIRefresh();
                }
            ...
            if (controller == GaiaConstants.EnvironmentControllerType.Custom)
            {
                ...
                {
                    EditorGUILayout.HelpBox(m_editorUtils.GetTextValue("CustomPlayerEditorRunning"), MessageType.Warning);
                }

                m_profile.m_instantiateCustomControllers = m_editorUtils.Toggle("InstantiateCustomControllers", m_profile.m_instantiateCustomControllers, helpEnabled);
                if (m_profile.m_customPlayer == null)
                {
                ...
                if (m_profile.m_customCamera == null)
                {
                    GUI.backgroundColor = Color.red;
                }
                ...
                if (m_profile.m_customCamera == null || m_profile.m_customPlayer == null)
                {
                    EditorGUILayout.HelpBox(m_editorUtils.GetTextValue("MissingCutomPlayerAndCamera"), MessageType.Error);
                    GUI.enabled = false;
                }

                if (m_editorUtils.Button("ApplySetup"))
                {
                    GaiaSceneManagement.CreatePlayer(m_gaiaSettings, m_profile.m_instantiateCustomControllers, "Custom", m_profile.m_spawnPlayerAtCurrentLocation, m_profile.m_customPlayer, m_profile.m_customCamera, true);
                    SetPostProcessing(profile);
#if GAIA_PRO_PRESENT
                    if (GaiaUtils.HasDynamicLoadedTerrains())
                    {
        ...
        private void RemoveCustomControllers()
        {
            if (m_profile.m_customCamera != null)
            {
                DestroyImmediate(m_profile.m_customCamera.gameObject);
            }

            if (m_profile.m_customPlayer != null)
            {
                DestroyImmediate(m_profile.m_customPlayer);
            }
        }
        ...
        private void AutoDepthOfField(bool helpEnabled)
        {
            if (m_gaiaSettings.m_currentRenderer == GaiaConstants.EnvironmentRenderer.BuiltIn)
            {
#if !UNITY_POST_PROCESSING_STACK_V2 && !UPPipeline 
            ...
            if (autoFocus != null && m_profile.m_enableAutoDOF)
            {
                ...
                if (autoFocus.m_interactWithPlayer)
                {
                    EditorGUI.indentLevel++;
                    autoFocus.m_playerTag = m_editorUtils.TextField("PlayerTag", autoFocus.m_playerTag, helpEnabled);
                    EditorGUI.indentLevel--;
                }
        ...
        private void CameraCullingSettings(bool helpEnabled)
        {
            ...
            if (m_profile.m_enableLayerCulling)
            {
                ...
                if (ProceduralWorldsGlobalWeather.Instance == null)
                {
                    ...
                    {
                        m_profile.m_sunLight = GaiaUtils.GetMainDirectionalLight(false);
                    }
                }
#else
                m_profile.m_sunLight = (Light)m_editorUtils.ObjectField("SunLight", m_profile.m_sunLight, typeof(Light), true, helpEnabled);
