    ...
    public class GaiaWaterEditor : PWEditor
    {
            ...
            if (mode != m_profile.m_waterSystemMode)
            {
                ...
                switch (m_profile.m_waterSystemMode)
                {
                    case GaiaConstants.GlobalSystemMode.Gaia:
                    {
                        ...
                        {
                            GaiaWater.GetProfile(m_profile.m_selectedWaterProfileValuesIndex, m_profile, true, false);
            ...
            if (m_profile.m_waterSystemMode == GaiaConstants.GlobalSystemMode.Gaia)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (m_profile.m_waterUpdateInRealtime)
                    {
                        if (m_profile.m_selectedProfile != "None")
                        {
                            GaiaWater.GetProfile(m_profile.m_selectedWaterProfileValuesIndex, m_profile, true, updateSettingsOnly);
                            updateSettingsOnly = false;
                        }
                    }
                }
            ...
            if (m_profile.m_enableReflections)
            {
                ...
                if (m_renderPipeline != GaiaConstants.EnvironmentRenderer.BuiltIn)
                {
                     ...
                     else
                     {
                         ...
                         // m_profile.m_reflectShadowsHDRP = m_editorUtils.Toggle("ReflectShadowsHDRP", m_profile.m_reflectShadowsHDRP, helpEnabled);
                         // if (m_profile.m_reflectShadowsHDRP)
                         // {
                         //     EditorGUILayout.HelpBox(m_editorUtils.GetTextValue("ReflectShadowsHDRPInfo"), MessageType.Info);
                         // }
                         m_profile.m_roughReflectionsHDRP = m_editorUtils.Toggle("RoughReflectionsHDRP", m_profile.m_roughReflectionsHDRP, helpEnabled);
#endif
                         m_profile.m_reflectionResolution = (GaiaConstants.GaiaProWaterReflectionsQuality)m_editorUtils.EnumPopup("ReflectionResolution", m_profile.m_reflectionResolution, helpEnabled);
                         m_profile.UpdateTextureResolution();
                         m_profile.m_reflectedLayers = LayerMaskField("ReflectedLayers", m_editorUtils, m_profile.m_reflectedLayers);
        ...
        private void WaterMeshQualityEnabled(bool helpEnabled)
        {
            ...
            if (m_profile.m_enableWaterMeshQuality)
            {
                ...
                else if (m_profile.m_meshType == GaiaConstants.MeshType.Circle)
                {
                    m_profile.m_xSize = m_editorUtils.IntField("WaterSize", m_profile.m_xSize, helpEnabled);
                    m_profile.m_zSize = m_profile.m_xSize;
                }
                ...
                {
                    m_profile.m_customWaterMesh = (Mesh)m_editorUtils.ObjectField("CustomWaterMesh", m_profile.m_customWaterMesh, typeof(Mesh), false, helpEnabled);
                }
                EditorGUI.indentLevel--;

                if (m_profile.m_meshType != GaiaConstants.MeshType.Custom)
                {
                    ...
                    EditorGUI.indentLevel++;
                    if (m_profile.m_waterMeshQuality == GaiaConstants.WaterMeshQuality.Custom && m_profile.m_meshType != GaiaConstants.MeshType.Custom)
                    {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    if (m_profile.m_waterUpdateInRealtime)
                    {
                        GaiaWater.UpdateWaterMeshQuality(m_profile, m_profile.m_waterPrefab, true);
                    }
                }
            }
        }
        ...
        private void WaterProfileSettingsEnabled(bool helpEnabled)
        {
            ...
            if (!m_profile.m_waterUpdateInRealtime)
            {
                if (m_editorUtils.Button("UpdateToScene"))
                {
                    if (m_profile.m_selectedProfile != "None")
                    {
                        GaiaWater.GetProfile(m_profile.m_selectedWaterProfileValuesIndex, m_profile, true, true);
                    }
                }
            }
        ...
        private void CausticSettingsEnabled(bool helpEnabled)
        {
            ...
            if (m_profile.m_useCastics)
            {
                ...
                {
                    m_profile.m_mainCausticLight = GaiaUtils.GetMainDirectionalLight(false);
                }
        ...
        private void ThirdPartyPanel(bool helpEnabled)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            m_profile.m_thirdPartyWaterObject = (GameObject)m_editorUtils.ObjectField("ThirdPartySystem", m_profile.m_thirdPartyWaterObject, typeof(GameObject), true);
            ...
            if (m_profile.m_thirdPartyWaterObject == null)
            {
                EditorGUILayout.HelpBox(m_editorUtils.GetTextValue("MissingThirdPartyObject"), MessageType.Error);
            }
        }
        /// <summary>
        ...
        private void AddNewCustomProfile()
        {
            ...
            {
                GaiaWater.GetProfile(newProfileListIndex, m_profile, true, true);
            }
