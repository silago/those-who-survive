...
namespace Gaia
{
    ...
    public class GaiaPlanarReflections : MonoBehaviour
    {
        ...
        {
            if (m_reflectionTexture == null || m_reflectionTexture.width != m_settings.m_textureResolution)
            {
                m_reflectionTexture = CreateReflectionTexture();
            }
...
#if UPPipeline
            Vector2Int res = ReflectionResolution(m_settings.m_textureResolution, UniversalRenderPipeline.asset.renderScale);
            const RenderTextureFormat hdrFormat = RenderTextureFormat.DefaultHDR;
            return new RenderTexture(res.x, res.y, 24, GraphicsFormatUtility.GetGraphicsFormat(hdrFormat, true));
#else
            return null;
#endif
        }
