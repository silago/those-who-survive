...
namespace Gaia
{
    ...
    public class GaiaGlobal : MonoBehaviour
    {
        ...
        public static void SetupNetworkPlayerAndCamera(GameObject player, Camera camera, bool closePhotoMode)
        {
            if (player != null)
            {
            ...
            if (camera != null)
            {
                FinalizeCameraObjectRuntime(camera, closePhotoMode);
            }
            else
            {
                    ...
                    if (session.m_session != null)
                    {
                        if (playerObj.transform.position.y < session.m_session.m_seaLevel)
                        {
                            playerObj.transform.position = new Vector3(playerObj.transform.position.x,
                                session.m_session.m_seaLevel + 5f, playerObj.transform.position.z);
                        }
                    }
                }
            }
        ...
        public static void FinalizeCameraObjectRuntime(Camera cameraObject, bool closePhotoMode)
        {
            ...
            GaiaPlanarReflectionsHDRP reflectionsHDRP = FindObjectOfType<GaiaPlanarReflectionsHDRP>();
            if (reflectionsHDRP != null)
            {
                reflectionsHDRP.m_mainCamera = cameraObject;
            }

            //PhotoMode photoMode = PhotoMode.Instance;
            //if (photoMode != null && closePhotoMode)
            //{
            //    photoMode.ClosePhotoMode();
            //}

            UIConfiguration gaiaUI = UIConfiguration.Instance;
            if (gaiaUI != null)
            {
                gaiaUI.SetFreeCameraComponent(cameraObject);
            }

#if HDPipeline
            HDRPDensityVolumeController densityVolumeController = HDRPDensityVolumeController.Instance;
            if (densityVolumeController != null)
            {
                densityVolumeController.m_mainCamera = cameraObject;
            }
#endif
        }
